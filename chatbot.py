...
class AnzarAssistant:
    ...
    def generate_response(self, query: str, context: str) -> str:
        # Define the prompt for Anzar to respond in Arabic based on the context
...
class RAGPipeline:
    def __init__(self, index_name='water-laws', dimension=384, model_url="https://api-inference.huggingface.co/models/meta-llama/Meta-Llama-3-8B-Instruct"):
        ...
        # Load the HuggingFace API key from a file
    ...
    def process_query(self, query):
        # Step 1: Retrieve relevant documents using the RAG model
        contexts = self.rag.retrieve_relevant_context(query, top_k=4)  # Adjust top_k for more context
        
        if not contexts:
            return "لم يتم العثور على معلومات ذات صلة."

        # Step 2: Format context and query for the LLM
        context_str = "\n".join([f"- {context}" for context in contexts])
        formatted_query = f"السياق ذو الصلة:\n{context_str}\n\nالسؤال: {query}"

        # Step 3: Get the LLM response based on the query and context using AnzarAssistant
